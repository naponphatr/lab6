...
@export var pull_acceleration := 18.0
...
func follow_player(delta):
	# Speed ramps up the longer it's being pulled in, so it feels like a snappy "suck in" rather than a slow drift.
	follow_speed += pull_acceleration * delta
